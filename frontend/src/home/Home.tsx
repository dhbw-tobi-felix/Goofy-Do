...
export default function Home() {
    ...
    if (!authenticated) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[80vh] w-full p-4 animate-in fade-in zoom-in-95 duration-500">
                <div className="w-full max-w-md bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-2xl text-center">
    ...
    return (
        <div className="container mx-auto max-w-[870px] p-5 md:py-10 min-h-screen flex flex-col">
            ...
            {/* Header Bereich: Ersetzt .topbar */}
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
                <div className="text-center sm:text-left">
                    <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight">Deine Listen</h1>
            ...
            {/* Content Bereich: Ersetzt .card mit responsivem Grid */}
            {loading ? (
                ...
                <>
                    {filteredLists.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-24 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl bg-zinc-50/50 dark:bg-zinc-900/30">
                            ...
                            {!searchTerm && (
                                <button onClick={() => setCreateModalOpen(true)} className="text-blue-600 hover:text-blue-700 font-bold hover:underline">
                                    Jetzt starten
                                </button>
                            )}
                        </div>
                                ...
                                >
                                    <div>
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 p-2.5 rounded-xl">
                                                <FolderOpen size={20}/>
                                            </div>
                                            <ChevronRight className="text-zinc-300 group-hover:text-zinc-600 dark:text-zinc-700 dark:group-hover:text-zinc-400 transition-colors" size={20}/>
                                        </div>
                                        ...
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400 line-clamp-2 h-10 leading-relaxed">
                                            {list.description || <span className="opacity-40 italic">Keine Beschreibung</span>}
                                        </p>
                                    </div>
                                    <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between text-xs text-zinc-400 font-medium">
                                        <span>Öffnen</span>
                                        <div className="w-6 h-6 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                            <ArrowRightIcon />
                                        </div>
                                    </div>
                                </Link>
            ...
            >
                <div className="space-y-5 py-2">
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">Name der Liste</label>
                        <input
                            autoFocus
                            value={nListName}
                    ...
                    <div>
                        <label className="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1.5 ml-1">Beschreibung (Optional)</label>
                        <textarea
                            value={nListDesc}
